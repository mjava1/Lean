        ...
        private bool SubmitOrder(string endpoint, Order order)
        {
            ...
            if (response.StatusCode == HttpStatusCode.OK)
            {
                ...
                if (string.IsNullOrEmpty(raw?.Id))
                {
                    ...
                    OnOrderEvent(new OrderEvent(order, DateTime.UtcNow, OrderFee.Zero, "Bitfinex Order Event") { Status = OrderStatus.Invalid, Message = errorMessage });
                ...
                OnOrderEvent(new OrderEvent(order, DateTime.UtcNow, OrderFee.Zero, "Bitfinex Order Event") { Status = OrderStatus.Submitted });
            ...
            OnOrderEvent(new OrderEvent(order, DateTime.UtcNow, OrderFee.Zero, "Bitfinex Order Event") { Status = OrderStatus.Invalid });
