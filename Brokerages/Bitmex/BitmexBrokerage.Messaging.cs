    ...
    public partial class BitmexBrokerage
    {
        ...
        private void OnOrderClose(string[] entries)
        {
            ...
            if (entries[5].IndexOf("canceled", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                ...
                {
                    OnOrderEvent(new OrderEvent(order, DateTime.UtcNow, OrderFee.Zero, "Bitfinex Order Event") { Status = OrderStatus.Canceled });
                }
            }
            else
            {
        ...
        private void EmitFillOrder(string[] entries)
        {
            try
            {
                ...
                (
                    ...
                    OrderFee.Zero, $"Bitfinex Order Event {direction}"
                );
